...
line, = ax.plot([], [], marker='o', linestyle='-', color='r', label='Line sorted by Key')
...
def update(frame):
    x_data.append(sorted_bpm_values_by_key[frame])
    y_data.append(sorted_key_numeric_values_by_key[frame])
...
ani.save(f"/home/user/Documents/6IX SENSE/ESSENTIA ANALYZER/figures/bpm_vs_key_animation_by_key_{datetime.now().strftime('%Y%m%d_%H%M%S')}.gif", writer='imagemagick')
